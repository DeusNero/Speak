...
document.getElementById('thoughts-select-delete').addEventListener('click',()=>{
    ...
    var delIds=[...thoughtsSelected];
    captures=captures.filter(c=>!thoughtsSelected.has(c.id));
    saveCaptures();
    if(typeof sbDeleteThoughts==='function')sbDeleteThoughts(delIds);
...
document.getElementById('confirm-action').addEventListener('click',()=>{var delId=currentDetailId;captures=captures.filter(c=>c.id!==delId);saveCaptures();if(typeof sbDeleteThought==='function')sbDeleteThought(delId);document.getElementById('confirm-overlay').classList.remove('visible');showScreen('thoughts-screen');});
...
document.getElementById('detail-refine').addEventListener('click',async()=>{if(_isRefining)return;_isRefining=true;const c=captures.find(x=>x.id===currentDetailId);if(!c){_isRefining=false;return;}const ov=document.getElementById('refine-overlay'),bd=document.getElementById('refine-body'),ac=document.getElementById('refine-actions');bd.innerHTML='<div class="refine-label">Original</div><div class="refine-original">'+escapeHtml(c.text)+'</div><div class="refine-label">Refined</div><div class="refine-loading"><div class="refine-loading-spinner"></div>Refining your text...</div>';ac.style.display='none';ov.classList.add('visible');pushNav('refine-overlay');const result=await refineText(c.text,c.lang||'en-US');_isRefining=false;let statusHtml='';if(typeof result==='object'){if(result.source==='local'&&result.error){statusHtml='<div style="font-size:12px;color:var(--mood-1);margin-bottom:12px;">Gemini unavailable: '+escapeHtml(result.error)+'. Showing local cleanup.</div>';}else if(result.source==='local'){statusHtml='<div style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">No API key. Showing local cleanup.</div>';}else{statusHtml='<div style="font-size:12px;color:var(--accent-moss);margin-bottom:12px;">Refined with Gemini AI</div>';}}const refinedText=typeof result==='object'?result.text:result;bd.innerHTML='<div class="refine-label">Original</div><div class="refine-original">'+escapeHtml(c.text)+'</div><div class="refine-label">Refined</div><div class="refine-refined" id="refined-text" contenteditable="true">'+escapeHtml(refinedText)+'</div>'+statusHtml;ac.style.display='flex';});
